...
$ProgramFiles = @(
    ...
    'tomato_source.py', 'index.html', 'requirements.txt', 'scripts'
)
...
function Start-App {
    Start-Process -FilePath $PythonExe -WorkingDirectory $ProjectDir `
        ...
        -RedirectStandardError (Join-Path $LogDir 'server-error.log') -WindowStyle Hidden | Out-Null
}
function Test-Code {
...
function Copy-DeploymentFiles([string]$SourceRoot, [string]$DestinationRoot) {
    ...
    if (Test-Path $builtinSource) {
        ...
        Copy-Item (Join-Path $builtinSource '*') $builtinDestination -Recurse -Force
    }
}
...
function Worker {
    ...
    $work = Join-Path $env:TEMP ("novel-oss-deploy-" + [guid]::NewGuid().ToString('N'))
    $archive = Join-Path $work 'latest.zip'
    ...
    try {
        ...
        if ($ChecksumUrl) {
            $checksumFile = Join-Path $work 'latest.zip.sha256'
            Invoke-WebRequest -UseBasicParsing -Uri $ChecksumUrl -OutFile $checksumFile -TimeoutSec 30
        ...
        if (Test-Path $StateFile) {
            try {
                if ([string](Get-Content $StateFile -Raw | ConvertFrom-Json).package_sha256 -eq $hash) {
                    Log "unchanged package_sha256=$hash"; return
                }
            } catch {}
        }
        Expand-Archive -Path $archive -DestinationPath $extract -Force
        foreach ($required in @('server.py', 'index.html', 'VERSION')) {
            if (-not (Test-Path (Join-Path $extract $required))) { throw "Package is missing $required." }
        }
        ...
        try {
            ...
            Test-Code
            & $PythonExe (Join-Path $ProjectDir 'scripts\sync_sources.py') --probe-key 'test' >> $DeployLog 2>&1
            $syncCode = $LASTEXITCODE
            Stop-App; Start-App; Test-Health
            @{ version=$version; package_sha256=$hash; deployed_at=(Get-Date).ToString('s'); sync_exit=$syncCode } |
