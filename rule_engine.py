# -*- coding: utf-8 -*-
"""书源规则引擎（兼容 Legado 书源 JSON 的常用子集）。
...
# JSONPath 表达式编译缓存：jsonpath-ng 的 parse 很慢（~80ms/次），
# 长目录（上千章）反复求值同一表达式时必须复用编译结果。
...
class UnsupportedRule(RuleError):
    """规则用到了本引擎不支持的能力（如内嵌 JS）。"""


# --------------------------------------------------------------------------
# URL 规格解析：支持 Legado 的  "url,{...json选项...}"  写法
# --------------------------------------------------------------------------
...
def _replace_vars(text: str, key: str | None, page: int, charset: str | None) -> str:
    ...
    key = key or ""
    mapping = {
        "{{key}}": _encode(key, charset),
        "{{page}}": str(page),
        "searchKey": _encode(key, charset),
        "searchPage": str(page),
    }
    out = text
...
def parse_url_spec(spec: str, key: str | None = None, page: int = 1) -> dict:
    ...
    return {
        ...
        "charset": charset,
        "headers": headers,
        "webView": bool(options.get("webView")),
    }


def _safe_referer(value: str | None) -> str | None:
    """Referer 只能承载 Latin-1；去掉书源 URL 中常见的 emoji fragment。"""
    if not value:
        return None
    try:
...
def http_fetch(session: requests.Session, spec: str, base_headers: dict | None = None,
               key: str | None = None, page: int = 1, timeout: int = 20,
    ...
    resp = session.request(
        info["method"], info["url"], data=info["body"], headers=headers,
...
def build_context(text: str) -> Any:
    """把响应文本变成可供规则查询的上下文（JSON 对象或 HTML 树）。"""
...
def _node_text(node: Any, attr: str | None) -> str:
    ...
    if attr == "owntext":
        parts = [node.text or ""] + [c.tail or "" for c in node]
        return " ".join(p.strip() for p in parts if p.strip())
    if attr in ("html", "innerhtml", "outerhtml"):
        raw = etree.tostring(node, encoding="unicode", method="html")
        return raw.strip()
    value = node.get(attr)
    if value is None and attr == "href":
        value = node.get("data-href") or node.get("data-url")
    if value is None and attr == "src":
        value = node.get("data-src") or node.get("data-original")
    return (value or "").strip()


def _apply_replaces(value: str, tails: list[str]) -> str:
    if not value or not tails:
        return value
    if len(tails) == 1:
        return re.sub(tails[0], "", value)
    for i in range(0, len(tails) - 1, 2):
        pattern = tails[i]
        repl = tails[i + 1].replace("$", "\\")
        try:
...
def _default_tokens(rule: str) -> list[str]:
    return [t for t in rule.split("@") if t != ""]


def _step_default(nodes: list, token: str) -> list:
    """执行 Legado 默认规则的一步，如 class.list.0 / tag.li / children"""
    low = token.lower()
    if low == "children":
        out = []
        for node in nodes:
    ...
    if kind == "class":
        ...
        css = f":contains('{name}')" if name else None
    else:
        css = token  # 直接当 CSS 处理
...
def _select_nodes(ctx: Any, core: str) -> list:
    ...
    if core.startswith("json:") or core.startswith("$."):
        ...
        if jsonpath_parse is None:
            raise RuleError("缺少 jsonpath-ng，无法解析 JSON 规则")
        if not isinstance(ctx, (dict, list)):
            raise RuleError("当前响应不是 JSON，无法使用 json 规则")
        return [m.value for m in _jsonpath_compiled(expr).find(ctx)]

    if isinstance(ctx, (dict, list)):
        raise RuleError("当前响应是 JSON，请使用 json:$.xxx 规则")

    if core.startswith("xpath:") or core.startswith("//") or core.startswith("./"):
        expr = core[6:] if core.startswith("xpath:") else core
        try:
            return list(ctx.xpath(expr))
        except Exception as exc:
            raise RuleError(f"XPath 无效：{expr} ({exc})") from exc

    if core.startswith("regex:"):
        raw = etree.tostring(ctx, encoding="unicode", method="html")
        return [m.group(1) if m.groups() else m.group(0)
                for m in re.finditer(core[6:], raw, re.S)]

    if core.startswith("css:"):
        css = core[4:].strip()
        try:
            return list(ctx.cssselect(css))
        except Exception as exc:
            raise RuleError(f"CSS 无效：{css} ({exc})") from exc

    # 默认规则（Legado 风格）
    nodes = [ctx]
    for token in _default_tokens(core):
        if token.lower() in TEXT_ATTRS:
            break
...
def _split_attr(core: str) -> tuple[str, str | None]:
    """从规则里拆出末尾的属性名（css/默认规则用）。"""
    if core.startswith(("xpath:", "//", "./", "json:", "$.", "regex:")):
        return core, None
    ...
    if tail_l in TEXT_ATTRS or re.fullmatch(r"[A-Za-z_:][\w:.\-]*", tail.strip()):
        if "." in tail and tail_l not in TEXT_ATTRS and not tail.startswith("data-"):
            # 形如 class.xxx 的结构 token，不是属性
            return core, None
...
def get_string(ctx: Any, rule: str | None, base_url: str = "", join: str = " ") -> str:
    ...
    if rule is None or str(rule).strip() == "":
        return ""
    rule = str(rule)
    if any(marker in rule for marker in JS_MARKERS):
        raise UnsupportedRule("规则内含 JS，本引擎不支持")

    for alt in rule.split("||"):
        core, tails = _split_core_tails(alt)
        if not core:
            continue
        # url: 前缀 —— 不从响应体取值，而是对当前请求地址做 ## 正则替换。
        # 用于详情接口异常（如返回“书籍已下架”空数据）时，仍能由 URL 参数推导目录地址。
        ...
        value = _apply_replaces(value, tails)
        value = value.strip()
        if value:
...
def absolute(url: str, base_url: str) -> str:
    if not url:
        return ""
    # 部分书源会把相对路径拼接成带空格的残缺 URL，例如 "50881 /50881"。
    # URL 规则中的空白通常是误差，清理后再交给 urljoin，避免请求 404。
