...
- **聚合模式**：检测到 `/api/aggregator` 后端时启用，同时聚合「番茄 + 幻梦轻小说」两源，搜索结果分组展示、单源失败不影响另一源；
- **直连模式**：静态托管（GitHub Pages 等）自动回退，仅番茄源，行为与旧版一致。

部署聚合版（免费）：把本仓库接入 [Netlify](https://app.netlify.com)（Import from Git），其余全自动；`netlify.toml` 已配置静态目录与函数，后端零依赖零密钥。
